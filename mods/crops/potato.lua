...
for _,row in ipairs(rainbow.colours) do
    minetest.register_node("crops:soil_with_potatoes_"..row[1], {
        description = row[2].." Soil with potatoes",
        tiles = { "plain_dirt.png^[colorize:"..row[3].."^crops_potato_soil.png", "plain_dirt.png^[colorize:"..row[3] },
        sunlight_propagates = false,
        use_texture_alpha = false,
        walkable = true,
        groups = { snappy=3, flammable=3, oddly_breakable_by_hand=2, colourable = 1, soil=1, not_in_creative_inventory = 1 },
        paramtype2 = "facedir",
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local below = { x = pos.x, y = pos.y - 1, z = pos.z }
        local b_name = minetest.get_node(below).name
		if not minetest.registered_nodes[b_name].groups.soil then
			return
		end
		local meta = minetest.get_meta(pos)
		local damage = meta:get_int("crops_damage")
        local col = rainbow.get_color(b_name)
        if col ~= nil then
            minetest.set_node(below, { name = "crops:soil_with_potatoes_"..col })
        end
